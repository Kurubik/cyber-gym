# Contributing to Cyber Gym

Thanks for taking a look! Cyber Gym is intentionally small and dependency-light, and the goal is
...
in CI, which builds them from `main` after the merge; if your change needs an
APK to be judged, say so in the PR and a maintainer runs that build.

Every pull request runs the three test suites (frontend, MCP, api), with the results and the coverage
...
One CI quirk: a pull request from a fork runs its workflow in *your* fork, which usually has
...
(the workflow waits for a maintainer to approve it until then). Changes to the workflow files or
...
| You have | Goes to |
| --- | --- |
| A question, or self-hosting that won't behave | [An issue labelled `question`](https://github.com/Kurubik/cyber-gym/issues) |
| An idea you're not sure about yet | [An issue labelled `idea`](https://github.com/Kurubik/cyber-gym/issues) |
| A reproducible bug | [Issues](https://github.com/Kurubik/cyber-gym/issues) |
| A change you've already built | [A pull request](https://github.com/Kurubik/cyber-gym/pulls) |
