...
// The product speaks Russian, so these are the strings the copy interpolates — the platform
// names that are genuinely proper nouns stay as they are.
export const BIO = IS_APPLE ? 'Face ID / Touch ID' : IS_ANDROID ? 'отпечаток или лицо' : 'отпечаток пальца, лицо или PIN'
export const VAULT = IS_APPLE ? 'iCloud Keychain' : IS_ANDROID ? 'Google Password Manager' : 'твой менеджер паролей'
 ...
 * thing that assumed the site root was the API call. A reverse proxy that puts Cyber Gym under a
