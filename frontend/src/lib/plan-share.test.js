...
describe('plan-share units', () => {
  ...
  it('keeps valid legacy bundles without a unit in their destination-unit semantics', () => {
    const legacy = {
      cybergym_plan: 1, name: 'Legacy', summary: 'old export',
      week: { 1: 'r' }, customEx: [],
  ...
  it('accepts legacy root omissions and the older weightUnit marker', () => {
    const legacy = {
      cybergym_plan: 1, weightUnit: 'lbs', legacyNote: 'kept as metadata',
      routines: [{ id: 'r', name: 'Strength', ex: [{ id: '0025', sets: 3, reps: 5, weight: 135, inc: 10 }] }],
    }
...
describe('what survives a shared plan', () => {
  ...
  it('clamps a hand-edited warm-up count instead of showing it verbatim', () => {
    const bundle = { cybergym_plan: 1, name: 'x', routines: [{ id: 'r', name: 'R', ex: [{ id: '0025', sets: 3, reps: 5, warmupSets: 999 }] }], week: {}, customEx: [] }
  ...
  it('normalises a hand-edited rest to a positive whole number or drops it', () => {
    const withRest = restSec => ({ cybergym_plan: 1, name: 'x', routines: [{ id: 'r', name: 'R', ex: [{ id: '0025', sets: 3, reps: 5, restSec }] }], week: {}, customEx: [] })
  ...
  it('clamps out-of-range intensifier numbers to the floors the app enforces', () => {
    const bundle = { cybergym_plan: 1, name: 'x', routines: [{ id: 'r', name: 'R', ex: [{ id: '0025', sets: 3, reps: 5, intensifier: { type: 'dropset', count: 0, pct: -5 } }] }], week: {}, customEx: [] }
  ...
  it('falls back to the default drop percentage when the file omits it', () => {
    const bundle = { cybergym_plan: 1, name: 'x', routines: [{ id: 'r', name: 'R', ex: [{ id: '0025', sets: 3, reps: 5, intensifier: { type: 'dropset' } }] }], week: {}, customEx: [] }
...
describe('week schedule as a routine-id list', () => {
  ...
  it('tolerates a legacy scalar bundle value', () => {
    const legacy = { cybergym_plan: 1, name: 'x', customEx: [], week: { 1: 'a' }, routines: twoRoutines.routines }
  ...
  it('scheduledDays counts a populated array day as 1 and a [] / absent day as 0', () => {
    expect(parsePlan({ cybergym_plan: 1, routines: [], customEx: [], week: { 1: ['a'], 2: [], 4: 'b' } }).scheduledDays).toBe(2)
  })
})
...
describe('custom exercises in a shared plan', () => {
  ...
  it('keeps importing an old name-plus-body-part file, editable at the other end', () => {
    const legacy = { cybergym_plan: 1, routines: [{ id: 'r', name: 'R', ex: [{ id: 'x', sets: 3, reps: 5 }] }], customEx: [{ id: 'x', n: 'Old one', bp: 'legs' }] }
    const target = { routines: [], week: {}, customEx: [] }
  ...
  it('drops muscles it cannot draw and a secondary that repeats a primary', () => {
    const bundle = { cybergym_plan: 1, routines: [], customEx: [{ id: 'x', n: 'Odd', bp: 'back', eq: 'barbell', primaries: ['upper-back', 'wings'], secondaries: ['upper-back', 'biceps', 7] }] }
    const target = { routines: [], week: {}, customEx: [] }
