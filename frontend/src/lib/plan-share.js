...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR Cyber Gym — just the
...
const unitError = () => { throw new Error(t('this isn’t an Cyber Gym plan file')) }
...
export function buildPlanBundle(S, name) {
  ...
  return { cybergym_plan: PLAN_FMT, exported: todayISO(), name: name || '', unit, week, routines, customEx }
}
...
export function parsePlan(raw, destinationUnit = 'kg') {
  ...
  if (!data || typeof data !== 'object' || Array.isArray(data) || !data.cybergym_plan || !Array.isArray(data.routines) || !destination) {
    throw new Error(t('this isn’t an Cyber Gym plan file'))
  }
...
<body><div class="doc">
  <header>
    <div class="kicker">Cyber Gym</div>
  ...
  ${body}
  <footer>${esc(t('Made with Cyber Gym'))} · cybergym.example.com</footer>
</div></body></html>`
}
