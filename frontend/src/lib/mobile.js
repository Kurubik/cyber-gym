...
const FILE = 'cyber-gym-state.json'
...
// device chose, kept in its own file — never inside cyber-gym-state.json, since that file's content
...
const REMOTE_FILE = 'cybergym-remote.json'
...
export async function writeAutoBackup(state) {
  try {
    ...
    await Filesystem.writeFile({
      path: `cyber-gym-backup-${todayISO()}.json`,
