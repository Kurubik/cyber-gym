...
// everything on this device, or connect to a self-hosted Cyber Gym server instead. See
...
export function ConnectSheet({ close }) {
  ...
  return <>
    ...
    <div className="muted small" style={{ marginBottom: 14 }}>
      {t('Open Settings → “Pair the mobile app” on the Cyber Gym site you’re already signed into, then enter its address and the code shown there.')}
    </div>
...
export default function MobileOnboarding() {
  ...
  const head = <>
    ...
    <h1 style={{ fontSize: 34, fontWeight: 700, letterSpacing: '-.028em', margin: '10px 0 4px' }}>Cyber Gym</h1>
  </>
  const wrap = { display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: '78vh', textAlign: 'center' }
  return (
    <div className="narrow" style={wrap}>
      {head}
      <div className="muted" style={{ marginBottom: 34 }}>{t('How do you want to use Cyber Gym?')}</div>
      ...
      <div className="dim small" style={{ marginTop: 26, lineHeight: 1.5 }}>
        {t('Local keeps everything on this phone. Connecting syncs to your own Cyber Gym server instead — you can switch later in Settings.')}
      </div>
    </div>
  )
}
