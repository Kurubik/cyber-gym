...
import { todayISO, localTZ, weekStartOf, MONDAY, SUNDAY } from '../lib/format.js'
...
export default function Settings() {
  ...
  const onUpdateRowClick = () => {
    ...
    if (updateInfo.apkUrl) {
      ...
      window.open('https://github.com/Kurubik/cyber-gym/releases/latest', '_blank', 'noopener')
    }
  }

  const doExport = async () => {
    const json = JSON.stringify(S, null, 2)
    const name = 'cyber-gym-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not an Cyber Gym backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? (user ? t('Your server') : t('Your data')) : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? (user ? <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Synced with your Cyber Gym server.')} />
        ...
        <Row icon="link" iconTint="var(--indigo)" title={t('Connect to my server')} subtitle={t('Sync this device to your own self-hosted Cyber Gym instead.')} accessory="chevron"
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host Cyber Gym')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
          onClick={() => window.open(REPO, '_blank', 'noopener')} />
      </> : user ? <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Signed in with passkey — data syncs to this profile.')} />
        {user.admin && <Row icon="wrench" iconTint="var(--indigo)" title={t('Admin dashboard')} accessory="chevron" onClick={() => nav('/admin')} />}
        <Row icon="link" iconTint="var(--blue)" title={t('Pair the mobile app')} subtitle={t('Connect the Cyber Gym app on your phone to this account.')} accessory="chevron"
    ...
    {MOBILE && <Section title={t('AI Coach')}>
      <Row icon="sparkles" iconTint="var(--acc)" title={t('AI Coach')} accessory="chevron"
        subtitle={coachLocal?.mode === 'server' ? t('Runs on your Cyber Gym server') : coachLocal?.mode === 'byok' ? t('Runs on this phone with your own API key') : t('Off — choose how the Coach should run.')}
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install Cyber Gym as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    {/* ---------- updates: the last thing on the page, so keeping Cyber Gym current is one tap ----------
    ...
    {(!MOBILE || android) && <Section title={t('Updates')}
      ...
      {MOBILE
        ? <Row icon="download" iconTint="var(--acc)"
            title={updateInfo?.hasUpdate ? t('Update to Cyber Gym v{0}', updateInfo.latestVersion) : t('Check for updates')}
        ...
        : <Row icon="download" iconTint="var(--acc)" title={t('Get the Android app')}
            subtitle={t('Download the APK from cybergym.example.com')} accessory="chevron"
            onClick={() => window.open('https://cybergym.example.com/#download', '_blank', 'noopener')} />}
    </Section>}
    ...
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      Cyber Gym v{__APP_VERSION__} · {t('free & open source (AGPL v3)')}<br />
      <a href="https://github.com/Kurubik/cyber-gym" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (MIT)<br />
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if Cyber Gym is closed.')}>
...
function PairSheet({ close }) {
  ...
  return <>
    ...
    <div className="muted small" style={{ marginBottom: 14 }}>
      {t('On the Cyber Gym app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.')}
    </div>
