...
// The weekday phrase is joined with the active date locale (Russian, since the product is
// Russian-only), so it is built the same way the sheet builds it instead of being hardcoded
// with an English list separator.
const dayPhrase = days => new Intl.ListFormat(dateLocale()).format(days)
...
describe('starter plan chooser', () => {
  ...
  it('asks first when a weekday the plan wants is already taken', () => {
    ...
    expect(confirm.textContent).toContain(dayPhrase(['Monday', 'Wednesday', 'Friday']))
