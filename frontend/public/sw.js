/* Cyber Gym service worker — the app shell and its hashed assets are cached at install and kept
...
const CACHE = 'cybergym-rt-__BUILD__'

// What the shell needs to boot without a network: index.html plus every script/style/icon it
// references, and the self-hosted fonts those stylesheets pull in. Read from the served
// index.html so the list follows the build, not a hand-kept manifest that would go stale the
// first time a chunk is renamed.
async function precache() {
  ...
  const refs = [...html.matchAll(/(?:src|href)="([^"]+)"/g)].map(m => m[1])
    .filter(u => /\.(?:js|css|png|svg|webmanifest|json|woff2)(?:\?|$)/.test(u) && !/^(?:https?:)?\/\//.test(u))
  const styles = [...new Set(refs.filter(u => /\.css(?:\?|$)/.test(u)))]
  await Promise.all([...new Set(refs)].map(u => c.add(u).catch(() => {})))
  // A font is only referenced from inside a stylesheet, so it is not in the HTML refs above.
  // Without this an app reopened offline (iOS especially) still boots, but in a fallback face.
  // The url() in a built stylesheet is relative to that stylesheet, not to this worker, so it is
  // resolved against the stylesheet's own URL before it is cached.
  const fonts = []
  for (const href of styles) {
    try {
      const sheet = new URL(href, self.location)
      const css = await (await c.match(sheet.href) || await fetch(sheet.href, { cache: 'no-cache' })).text()
      for (const m of css.matchAll(/url\((['"]?)([^'")]+\.woff2)\1\)/g)) fonts.push(new URL(m[2], sheet).href)
    } catch { /* a stylesheet we could not read simply contributes no fonts */ }
  }
  await Promise.all([...new Set(fonts)].map(u => c.add(u).catch(() => {})))
}
...
self.addEventListener('push', e => {
  e.waitUntil((async () => {
    ...
    const tag = data.tag || 'cybergym'
    try { for (const n of await self.registration.getNotifications({ tag })) n.close() } catch {}
    await self.registration.showNotification(data.title || 'Cyber Gym', {
      body: data.body || '',
      icon: 'icon-512.png',
      badge: 'icon-180.png',
      tag,
