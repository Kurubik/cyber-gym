# Cyber Gym MCP server

A [Model Context Protocol](https://modelcontextprotocol.io) bridge that lets an external LLM
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your Cyber Gym profile —
...
session secrets — it can only read the same `state-<uid>.json` files the Cyber Gym api already
writes.
...
CYBERGYM_UID=<your-uid> CYBERGYM_DATA=/path/to/Cyber Gym/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "cybergym": {
      "command": "node",
      "args": ["/absolute/path/to/Cyber Gym/mcp/src/index.js"],
      "env": {
        "CYBERGYM_DATA": "/absolute/path/to/Cyber Gym/data",
        "CYBERGYM_UID": "<your-uid>"   // optional — auto-detected if you have one profile
      }
    }
  }
}
...
Restart the client; you should see the Cyber Gym tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as Cyber Gym.
