#!/usr/bin/env node
/* Cyber Gym MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
// Fail fast on bad config so a misnamed CYBERGYM_DATA doesn't silently answer every call with
...
try {
  init()
  const u = getUser()
  console.error(`[cyber-gym-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[cyber-gym-mcp] ${e.message}`)
