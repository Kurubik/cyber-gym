...
# Cyber Gym does not redistribute or relicense them. See NOTICE.md.
set -euo pipefail
cd "$(dirname "$0")/.."
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
↓ Downloading exercise media (~140 MB) from github.com/hasaneyldrm/exercises-dataset
  ...
  Used under that dataset's terms, not Cyber Gym's AGPL; Cyber Gym does not redistribute them.
