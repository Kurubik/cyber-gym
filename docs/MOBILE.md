...
Cyber Gym ships in two flavors from the same codebase:
...
telemetry. State is mirrored from `localStorage` into `cyber-gym-state.json` in the app's
...
you can instead **connect it to a self-hosted Cyber Gym server** — your data then lives there,
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which Cyber Gym doesn't do.
...
Cyber Gym's mobile app is not on the Play Store or App Store, and that's a choice: no store
...
- **[cybergym.example.com](https://cybergym.example.com)** — the download page.
- **[GitHub Releases](https://github.com/Kurubik/cyber-gym/releases)** — every
  build under `cyber-gym-android/<version>/`, with a `.sha256` beside it. Direct link, no login:
  `https://github.com/Kurubik/cyber-gym/releases/download/v<version>/cyber-gym-<version>.apk`
- **[The GitHub release](https://github.com/Kurubik/cyber-gym/releases)** for that version,
  with the APK and its `.sha256` attached as release assets.
- **[The release page](https://github.com/Kurubik/cyber-gym/releases/latest)**, which also
...
Both come out of CI: the Android job in [`.github/workflows/`](../.github/workflows) runs
...
`https://github.com/Kurubik/cyber-gym/actions`
...
keytool -genkeypair -keystore my.keystore -alias cybergym -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias cybergym --out cyber-gym.apk aligned.apk
```
...
There is a documented iOS build path for exactly this: the
same mobile bundle, `xcodebuild archive` without a signing identity, and an *unsigned* `.ipa`
(plus `.sha256`) as job artifact — on a tag also under `cyber-gym-ios/<version>/` in the package
registry — for AltStore/Sideloadly users to sign with their own Apple ID. It needs a Mac: Xcode
does not run on Linux CI runners, and hosted macOS runners are not on the
...
- **License:** Cyber Gym is AGPL-3.0, which by itself sits badly with app-store terms of
