# Self-hosting Cyber Gym

Cyber Gym is two small containers (a web server and an API) plus a folder of your data.
...
Cyber Gym signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use Cyber Gym from your phone you need a real HTTPS hostname.
...
Put Cyber Gym behind something that terminates TLS for a hostname you control, then point it at
...
Cyber Gym only needs the browser to reach it over `https://gym.example.com`. If that proxy caps
...
The defaults assume Cyber Gym is the only thing here: a service called `api` on port 3000, and nginx
...
BASE_PATH=                 # subpath Cyber Gym is served under, e.g. /gym — see below; empty = site root
...
### Serving Cyber Gym under a subpath

Cyber Gym can live at `https://example.com/gym/` rather than on a host of its own. Which of the two
...
example.com {
    handle_path /gym/* {
        reverse_proxy cyber-gym-web:80
    }
}
```
...
the prefix to the container — Cyber Gym does not serve itself at the site root as well, and a copy
...
```bash
tar czf cyber-gym-backup-$(date +%F).tar.gz data/
```
...
Cyber Gym can push two kinds of alert to your phone/desktop, even when the app isn't open:
...
you about your pushes. Cyber Gym sends your `ORIGIN` by default; set `VAPID_SUBJECT=mailto:you@example.com`
...
```
docker compose logs api | grep 'cyber-gym-api on'
# cyber-gym-api on :3000 (rpID=gym.example.com, origin=https://gym.example.com)
```
