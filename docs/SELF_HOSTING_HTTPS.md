...
This is a rather complex topic of its own, so it lives apart from the [SELF_HOSTING.md](./SELF_HOSTING.md) which focuses on how to run Cyber Gym.
This document takes a more general approach on how to achieve "https at home", it is not necessarily focused on Cyber Gym and applies to self hosting in general.
...
1. DNS provider translates name to the local IP of homelab (e.g.: `https://cybergym.example.com -> http://192.168.0.100:8080`)
...
1. register with desec.io and choose a name (we take `myhomelab` as example)
    ...
    2. A wildcard certificate handles all certificates for that given host. e.g.: `cybergym.example.com` and `nextcloud.myhomelab.dedyn.io` can share the same certificates.
...
3. install caddy
    ...
    # reverse proxy config for cybergym
    cybergym.example.com {
        import desec
...
And that's about it. Now you should be able to access your cybergym instance from your local network via https with working passkeys.  
Be sure to update the Cyber Gym config accordingly (`RP_ID=cybergym.example.com`, `ORIGIN=https://cybergym.example.com` — see [SELF_HOSTING.md](./SELF_HOSTING.md#2-understand-the-passkey-requirement-important)).
...
- `https://cybergym.example.com -> http://192.168.0.100:8080`
