...
Cyber Gym is a self-hosted app: you run the server, you hold the data. This file says which
...
patched. A fix ships in the next release and in the `latest` images published to GHCR.
...
The project lives on GitHub. Use GitHub's private vulnerability reporting, or if you would
have **confidential issues**, and that is the private channel: open an issue at
<https://github.com/Kurubik/cyber-gym/security/advisories/new> to open a private advisory.
...
If you'd rather not put the details in GitHub at all, open a private advisory saying only
...
> `github.com/Kurubik/cyber-gym/security/advisories/new` no longer resolves.

Please don't put a working exploit in a *non-confidential* issue if it can be used against other
people's instances — use the private advisory form rather than a public ticket. Everything else
(a crash you can only trigger on your own box, a scanner warning) is fine as a normal issue.
...
- **The published images** `ghcr.io/kurubik/cyber-gym-api` and `ghcr.io/kurubik/cyber-gym-web`.
...
- The GitHub Pages website — static files only, it has no backend and serves no user data.
- Third-party content: the exercise image/GIF dataset and the CDN it's fetched from.

## Security model

Read this before hosting Cyber Gym for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host Cyber Gym for other
