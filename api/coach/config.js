/* Coach instance configuration — the one place that knows whether this instance offers the
   ...
   Cyber Gym does not interpret any provider's terms on a self-hoster's behalf. It just makes
...
function key() {
  ...
  keyCache = Buffer.from(crypto.hkdfSync('sha256', Buffer.from(secret, 'utf8'), Buffer.alloc(0), Buffer.from('cybergym-coach-v1'), 32));
  return keyCache;
}
